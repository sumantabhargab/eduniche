...
import { TOC_RAW_DATA } from "@/data/gate-cse-analysis";
...
const COVERAGE = [
  { year: 2024, name: "CSE: XS 1, 2 (Feb 3)", count: 24, marks: 60, note: "Partial" },
  { year: 2024, name: "CSE: XS 1, 2 (Feb 4)", count: 22, marks: 55, note: "Partial" },
  { year: 2025, name: "CSE: XS 1, 2", count: 20, marks: 50, note: "Partial" },
  { year: 2026, name: "CSE: XS 1, 2", count: 17, marks: 42, note: "Partial" },
];
...
export default function GateDashboardClient({
        ...
        <main className="max-w-6xl mx-auto px-4 sm:px-6 py-12">
          <div className="text-center">
            ...
            <p className="text-sm text-muted max-w-md mx-auto mb-6">
              ...
              This takes time — we need to ensure every question is properly
              sourced and classified.
            </p>
            <p className="text-xs text-muted-light">
              Check back soon, or let us know you&apos;re interested.
            </p>
          </div>
        </main>
      </>
  ...
  // Sort subjects by total marks descending
  const subjects = [...TOC_RAW_DATA].sort((a, b) => b.totalMarks - a.totalMarks);
  const totalMarksAll = subjects.reduce((sum, s) => sum + s.totalMarks, 0);

  // Total questions per type
  const totals = TOC_RAW_DATA.reduce(
    (acc, s) => {
      if (typeof s.questionTypes.mcq === "number") acc.mcq += s.questionTypes.mcq;
      if (typeof s.questionTypes.msq === "number") acc.msq += s.questionTypes.msq;
      if (typeof s.questionTypes.nat === "number") acc.nat += s.questionTypes.nat;
      return acc;
    },
    { mcq: 0, msq: 0, nat: 0 }
  );
          ...
          <div className="max-w-6xl mx-auto">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-6">
              <div className="p-3 border border-border">
                ...
                <p className="text-lg font-medium text-foreground">23</p>
              </div>
              <div className="p-3 border border-border">
              ...
              <div className="p-3 border border-border">
                ...
                <p className="text-lg font-medium text-foreground">2004–2026</p>
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="py-10 px-4 sm:px-6">
          <div className="max-w-6xl mx-auto">
            ...
            <p className="text-xs text-muted-light mb-6">
              Explore historical questions, topic trends, and priority estimates
              for each subject.
            </p>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {subjects.map((subject) => {
                const marksPercent = Math.round(
                  (subject.totalMarks / totalMarksAll) * 100
                );
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <p className="text-xs text-muted">
                    ...
                    have consistently received significant weight across most
                    years.
                  </p>
                </div>
                <div>
                  ...
                  <p className="text-xs text-muted">
                    The relative distribution of marks across subjects
                    shifts moderately between sessions. No single year
                    is representative of all years.
                  </p>
                </div>
                <div>
                  <h3 className="text-sm font-medium text-foreground mb-1">
                  ...
                  <p className="text-xs text-muted">
                    {totals.mcq} MCQs, {totals.msq} MSQs, {totals.nat} NAT — all
                    three types appear every year.
                  </p>
                </div>
              </div>
            </div>
        ...
        <section className="pb-16 px-4 sm:px-6">
          <div className="max-w-6xl mx-auto">
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-border">
                    <th className="text-left px-4 py-3 text-xs font-mono text-muted-light font-medium">
                      Year
                    </th>
                    <th className="text-left px-4 py-3 text-xs font-mono text-muted-light font-medium">
                      Session
                    </th>
                    <th className="text-right px-4 py-3 text-xs font-mono text-muted-light font-medium">
                      Questions
                    </th>
                    <th className="text-right px-4 py-3 text-xs font-mono text-muted-light font-medium">
                      Marks
                    </th>
                    <th className="text-center px-4 py-3 text-xs font-mono text-muted-light font-medium">
                      Status
                    </th>
                  </tr>
                </thead>
                <tbody>
                  {COVERAGE.map((row) => (
                    <tr key={row.name} className="border-b border-border/50 last:border-0">
                      ...
                      <td className="px-4 py-2.5 text-xs text-muted">{row.name}</td>
                      ...
                      <td className="px-4 py-2.5 text-center">
                        <span className="text-xs text-amber-600 bg-amber-50 px-1.5 py-0.5 border border-amber-200">
                          {row.note}
                        </span>
                      </td>
                    </tr>
            ...
            <p className="text-xs text-muted-light mt-3">
              Some session splits are partially processed. More detailed
              coverage is being added progressively.
            </p>
          </div>
        </section>
      </main>
