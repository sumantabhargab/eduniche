...
interface Stats {
  ...
  periodTotal: { hours: number; minutes: number };
  sessions: number;
  dailyGoal: number;
  goalProgress: number;
  todayMinutes: number;
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    const fetchStats = async () => {
      try {
        const res = await fetch(`/api/study/stats?period=${period}`);
        if (res.ok) {
    ...
    <div className="max-w-6xl mx-auto px-6 py-8 space-y-8">
      ...
      <div>
        ...
        <p className="text-muted mt-1">
          {stats ? (
            <>
              You&apos;ve studied <span className="text-foreground font-medium">
                {formatTime(stats.periodTotal.hours, stats.periodTotal.minutes)}
              </span> {period === "today" ? "today" : period === "week" ? "this week" : period === "month" ? "this month" : "all time"}
            </>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <StatCard label="Study Time" value={
          stats ? formatTime(stats.periodTotal.hours, stats.periodTotal.minutes) : "—"
        } />
        <StatCard label="Sessions" value={stats?.sessions?.toString() ?? "—"} />
        <StatCard label="Streak" value={stats ? `${stats.streak.current} days` : "—"} accent="🔥" />
        <StatCard label="Goal" value={stats ? `${stats.goalProgress}%` : "—"} />
      </div>
      ...
      <div>
        ...
        <StudyTimerDisplay userId={user.id} onSessionEnd={handleSessionEnd} />
      </div>
      ...
      <div className="bg-card border border-border rounded-2xl p-6 md:p-8">
        ...
        <p className="text-sm text-muted mt-3">— {quote.author}</p>
      </div>

      {/* Quick actions */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <QuickAction href="/library" label="Library" emoji="📚" />
        <QuickAction href="/leaderboard" label="Leaderboard" emoji="🏆" />
        <QuickAction href="/pricing" label="Upgrade" emoji="⭐" />
        <QuickAction href="/profile" label="Profile" emoji="👤" />
      </div>
    </div>
  );
}
...
function StatCard({ label, value, accent }: { label: string; value: string; accent?: string }) {
  return (
    <div className="bg-card border border-border rounded-2xl p-4 md:p-6">
      <div className="text-xs text-muted uppercase tracking-wider font-medium mb-2">{label}</div>
...
function StudyTimerDisplay({ userId, onSessionEnd }: { userId: string; onSessionEnd: () => void }) {
  ...
  const startTimer = async () => {
    try {
      const res = await fetch("/api/study/sessions", {
        ...
        body: JSON.stringify({ branch_id: "general", subject_id: "general" }),
      });
      if (res.ok) {
        const data = await res.json();
  ...
  const endTimer = async () => {
    ...
    if (sessionId) {
      try {
        const elapsed = pauseElapsedRef.current + (status === "running" ? Date.now() - startTimeRef.current : 0);
        await fetch(`/api/study/sessions/${sessionId}`, {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ duration_seconds: Math.floor(elapsed / 1000) }),
        });
        onSessionEnd();
      } catch (e) {
  ...
  const startTick = () => {
    ...
    tickRef.current = setInterval(() => {
      const elapsed = pauseElapsedRef.current + (Date.now() - startTimeRef.current);
      setFocusSeconds(Math.floor(elapsed / 1000));
    }, 1000);
  };
    ...
    <div className="bg-card border border-border rounded-2xl p-6 sm:p-8 text-center space-y-6">
      {!pageVisible && status === "running" && (
        <div className="text-sm text-amber-600 dark:text-amber-400">
          ⏸️ Timer paused — switch back to EduNeuro to resume
        </div>
      )}
